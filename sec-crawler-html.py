...
class SECDataDownloader:
    ...
    def __init__(self, data_path='./sec-data/data', start_date='1993-01-01', end_date='2023-06-30', user_agent="", html_sleep=0.25):
        ...
        self.html_sleep = html_sleep
        self.rate_limiter = RateLimiter(0.1)
        self.rate_limiter_html = RateLimiter(0.25)
    ...
    def download_filings(self, filing_type: SECFileType, use_html: bool):
        for (yr, qr) in tqdm(self.filing_periods, desc=f"Filtering and Downloading {filing_type.value}"):
            ...
            csv_html_file_path = path / 'html-missing.csv'

            df = pd.read_csv(csv_file_path)
            sec_filing = df[(df['Form Type'] == filing_type.value)]
            if not use_html and 'Downloaded' in sec_filing.columns:
                sec_filing = sec_filing[(sec_filing['Downloaded'] is False)]
            ...
            if not use_html:
                ...
                with ThreadPoolExecutor(max_workers=4) as executor:
                    futures = [
                        executor.submit(self.download_sec_file_html, file, path)
                        for file in sec_filing['Filename']
                    ]
                    wait(futures)
    ...
    def download_sec_file_html(self, file, path):
        next(self.rate_limiter_html)

        file_url = f"https://www.sec.gov/Archives/{file}"

        file_url = file_url.replace('.txt','')
        components = file_url.split('/')
        filename_without_extension = components[-1].split('.')[0]
        archiveindex = filename_without_extension.replace('-', '')  # Remove hyphens
        components[-1] = f"{archiveindex}/{components[-1]}-index-headers.html"

        index_header = '/'.join(components)
        
        
        a = file.split('/')
        local_file = path / f"{a[-2]}_{a[-1]}"
        local_file = local_file.with_suffix('.html')
        ...
        try:
            response = requests.get(index_header, headers=self.headers)
            if response.status_code == 200:
                soup = BeautifulSoup(response.content, "html.parser")
                html_file = soup.a.get("href")

                if html_file is not None:
                    components = index_header.split('/')
                    components[-1] = html_file
                    file_url = '/'.join(components)
                    
                    #print("found html_file", file_url)
                    ...
                    print(f"Cannot locate {file}")

            else:
                print(f"{response.status_code} bad happened here")
    ...
    def validate_downloaded_filings(self, filing_type: SECFileType):
        for (yr, qr) in tqdm(self.filing_periods, desc=f"Validating {filing_type.value}"):
            ...
            for index, row in sec_filing.iterrows():
                filename = row['Filename']
                filename_local = re.search(r'[^/]+$', filename).group()

                file_path = path / filename_local
            ...
            not_downloaded_count = len(sec_filing[sec_filing['Downloaded'] == False])
            total_count = len(sec_filing)
            self.logger.info(f"{yr}-{qr}: Number of files missed: {not_downloaded_count} out of {total_count}")
...
def main():
    ...
    downloader = SECDataDownloader(
        ...
        html_sleep=config.get("SECDataDownloader", "html_sleep")
    )
    ...
    if config.getboolean("SECDataDownloader", "download_filings"):
        use_html = config.getboolean("SECDataDownloader", "use_html")
        downloader.download_filings(filing_type=filing_type_enum, use_html=use_html)

    if config.getboolean("SECDataDownloader", "validate_downloads"):
        downloader.validate_downloaded_filings(filing_type=filing_type_enum)
