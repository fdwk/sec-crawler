...
class SECDataDownloader:
    ...
    def __init__(self, data_path='./sec-data/data', start_date='1993-01-01', end_date='2023-06-30', user_agent=""):
        ...
        self.rate_limiter = RateLimiter(0.25)
    ...
    def download_filings(self, filing_type: SECFileType, use_html: bool):
        for (yr, qr) in tqdm(self.filing_periods, desc=f"Filtering and Downloading {filing_type.value}"):
            ...
            if not use_html:
                ...
                for file in tqdm(sec_filing['Filename']): 
                    result = self.download_sec_file_html(file, path)
                    if result is not True:
                        print(f"failed {file}")
                        html_files_failed.append(file)        

                
                html_files_missing_df = pd.DataFrame({'missing': html_files_failed})
                html_files_missing_df.to_csv(csv_html_file_path, index=False)
